...
var Particle = function(args){
	...
	this.imgSize = args.imgSize || Math.random() * 50; 
	this.position = {
	...
	if(this.position.x >= (window.innerWidth - this.position.x)){
		this.position.x -= this.imgSize;
	}
	if(this.position.y >= (window.innerHeight - this.position.y)){
		this.position.y -= this.imgSize;
	}
	...
	this.draw = function(ctx){
		this.update();
		// ctx.beginPath();
		// ctx.arc(this.position.x, this.position.y, this.imgSize, PI2, false);
		// ctx.fillStyle = "white";
		// ctx.fill();
		// ctx.closePath();

		ctx.beginPath();
		ctx.drawImage(img, this.position.x, this.position.y, this.imgSize,this.imgSize);
		ctx.closePath();
	}
	...
	this.update = function(){
		...
		if (this.position.x > (canvas.width - this.imgSize) || this.position.x < 0){this.speed.x = this.speed.x * -1;}
		if (this.position.y > (canvas.height - this.imgSize) || this.position.y < 0){this.speed.y = this.speed.y * -1;}
	}
	...
	for (var i = 0; i < particles.length; i++) {
		...
		for (var j = i + 1; j < particles.length; j++) {
			...
			if(distance < distanceTrigger){
				context.beginPath();
				context.moveTo((particle_a.position.x + (particle_a.imgSize / 2)), (particle_a.position.y + (particle_a.imgSize / 2)));
				context.lineTo((particle_b.position.x + (particle_b.imgSize / 2)), (particle_b.position.y + (particle_b.imgSize / 2)));
